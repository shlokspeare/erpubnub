...
-spec new() -> #epn{}.
new() ->
    {ok, Origin} = ?WITH_DEFAULT(application:get_env(erpubnub, origin), ?DEFAULT_ORIGIN),
    {ok, PubKey} = ?WITH_DEFAULT(application:get_env(erpubnub, pubkey), ?DEFAULT_PUBKEY),
    {ok, SubKey} = ?WITH_DEFAULT(application:get_env(erpubnub, subkey), ?DEFAULT_SUBKEY),
    {ok, SecretKey} = ?WITH_DEFAULT(application:get_env(erpubnub, secretkey), ?DEFAULT_SECRETKEY),
    {ok, SSL} = ?WITH_DEFAULT(application:get_env(erpubnub, ssl), ?DEFAULT_SSL),
...
spawn_subscribe(EPN, Channel, Callback) ->
    {ok, spawn(erpubnub, subscribe, [EPN, Channel, Callback])}.
